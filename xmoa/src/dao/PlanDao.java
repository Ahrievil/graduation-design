...
public class PlanDao extends BaseDao{
	...
	public int insert(OaPlan plan){
		...
		return jt.update(sql,plan.getPlName(),plan.getPrname(),plan.getFename(),plan.getRname(),plan.getMoname(),plan.getPplid(),plan.getFlag(),plan.getPlCreateTime(),plan.getPlEndTime(),plan.getPlCreateDate(),plan.getPlMgr(),plan.getPlWorker(),plan.getPlDateNum(),plan.getPlLevel(),plan.getPlType(),plan.getPlHeadPlan(),plan.getPlStatus(),plan.getPlOther(),plan.getPlIsExceed());
	}
	...
	public int update(OaPlan plan){
		...
		return jt.update(sql,plan.getPlName(),plan.getPlHeadPlan(),plan.getPlEndDate(),plan.getPlWorker(),plan.getPlLevel(),plan.getPlType(),plan.getFlag(),plan.getRname(),plan.getMoname(),plan.getFename(),plan.getIsok(),plan.getPlOther(),plan.getPlid());
	}
